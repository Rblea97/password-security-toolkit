...
def format_analysis_text(analysis: PasswordAnalysis, verbose: bool = False) -> str:
    ...
    output.append(f"{Style.BRIGHT}Strength Score: {color}{analysis.strength_score}/100 [{rating_display}]{Style.RESET_ALL}")
    output.append("")
    ...
    online_time = analysis.estimated_crack_time.get('online_attack_100_per_second', 'N/A')
    offline_time = analysis.estimated_crack_time.get('offline_attack_10B_per_second', 'N/A')
    ...
    if analysis.breach_status.get('checked', False):
        if analysis.breach_status.get('found', False):
            ...
            output.append(f"  {Fore.RED}⚠️  FOUND in {count:,} data breaches!{Style.RESET_ALL}")
            output.append(f"  {Fore.RED}    ⚠️  Change this password immediately!{Style.RESET_ALL}")
        else:
            output.append(f"  {Fore.GREEN}✓ Not found in breach database{Style.RESET_ALL}")
    else:
...
def format_batch_summary(results: List[PasswordAnalysis]) -> str:
    ...
    output.append(f"  {Fore.RED}Weak:        {ratings['weak']:3d} ({ratings['weak']/total*100:.1f}%){Style.RESET_ALL}")
    output.append(f"  {Fore.YELLOW}Moderate:    {ratings['moderate']:3d} ({ratings['moderate']/total*100:.1f}%){Style.RESET_ALL}")
    output.append(f"  {Fore.GREEN}Strong:      {ratings['strong']:3d} ({ratings['strong']/total*100:.1f}%){Style.RESET_ALL}")
    output.append(f"  {Fore.BLUE}Very Strong: {ratings['very_strong']:3d} ({ratings['very_strong']/total*100:.1f}%){Style.RESET_ALL}")
    output.append("")
    
    # Breach statistics
    output.append(f"{Style.BRIGHT}Breach Statistics:")
    output.append(f"  Passwords in breaches: {Fore.RED}{breached_count}{Style.RESET_ALL} ({breached_count/total*100:.1f}%)")
    output.append("")
    ...
    for i, result in enumerate(weakest, 1):
        ...
        output.append(f"  {i}. Score: {color}{result.strength_score:3d}{Style.RESET_ALL} | Hash: {hash_preview}")
    output.append("")
