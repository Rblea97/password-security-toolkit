...
def get_recommendations(analysis: PasswordAnalysis) -> List[str]:
    ...
    if not criteria.get('min_length', False):
        if analysis.length < 8:
            recommendations.append("⚠️ CRITICAL: Password is too short. Use at least 8 characters (12+ recommended)")
    ...
    if not criteria.get('no_common_patterns', False):
        recommendations.append("⚠️ Contains common pattern (e.g., 'password', '123456'). Use unique password")
    ...
    if analysis.entropy_bits < 40:
        recommendations.append("Password entropy is low. Make it longer and more random")
...
def analyze_password(password: str, check_breach_status: bool = True) -> PasswordAnalysis:
    """
